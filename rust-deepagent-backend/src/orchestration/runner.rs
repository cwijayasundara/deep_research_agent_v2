...
use crate::agents::orchestrator::run_deep_research;
...
pub async fn run_daily_research(
    ...
    tracing::info!(
        ...
        "Starting deep agent research for {}",
        date
    );

    let result = match run_deep_research(
        &state.openai_client,
    ...
    .await
    {
        Ok(raw_markdown) => {
            ...
            tracing::info!(
                ...
                "Deep agent pipeline completed, parsing structured data..."
            );
        ...
        Err(err) => {
            ...
            tracing::error!(
                ...
                duration_s = duration,
                error = %err,
                "Deep agent research pipeline FAILED"
            );
    ...
    tracing::info!(
        "Deep agent research complete: {} in {:.1}s total",
        report.report_id,
        total_duration
    );
